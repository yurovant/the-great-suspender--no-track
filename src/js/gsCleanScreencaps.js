var gsCleanScreencaps = {
  ...
  addListener: (tabId) => {
    ...
    const listener = (details) => {
      try {
        ...
        if (gsCleanScreencaps.blacklist[host]) {
          return { cancel: true };
        }
      } catch (err) {
        gsUtils.log(
          'background',
          'error while trying to block in gsCleanScreencaps',
          err
        );
      }
    };
    ...
    gsCleanScreencaps.listeners[tabId] = () =>
  ...
  storageData: () => {
    return new Promise((res, _) => {
      chrome.storage.local.get('gsCleanScreencapsBlacklist', (storage) =>
        res(storage.gsCleanScreencapsBlacklist)
      );
    });
  },
  ...
  loadList: async () => {
    ...
    if (!stored || stored.time + 3600 * 24 * 30 <= new Date().getTime()) {
      ...
      try {
        ...
        gsUtils.log(
          'background',
          'error while loading blocklist for clean screencapture:',
          err
        );
      }
    } else {
